...
Lingua::RU::Zaliz::Inflect - Словоизменение слов русского языка по словарю А.А.Зализняка
Lingua::RU::Zaliz::Inflect - Russian inflection from the dictionary by A.A.Zaliznyak
...
sub wi2paradigm {
  ...
  while (($key,$value) = each %wfh) {
    my @wfh = @$value;
    if (@wfh > 1) { my %saw; @wfh = grep(!$saw{join("", %$_)}++, @wfh) }
    $wfh{$key} = \@wfh;
  }
...
sub get_all_wi {
  ...
  if ($wi{'т'} ne "г") {
    ...
    if ($wi{'т2'} =~ /^[см]/) {
      (defined $wi{'ч'}) && ($wb =~ /[^${v}йь]$/ || $wb =~ s/[йь]$//) &&
        (($wb,$wi{'сб2'}) = (rem_vowel($wb, \%wi),$wb)) || ($wb =~ s/[${v}йь]$//)
    }
    if ($wi{'т2'} =~ /^[а]/) { # ($wi{'т2'} =~ /^[ам]/)
...
sub decline {
  ...
  if (defined($exc = $wi{'искл'})) {
    if (defined($excp = $exc->{$po})) {
      ...
      @wfh = @{$excp};
      goto LABEL1;
    } elsif ($p eq "тм" && defined($excp = $exc->{(($t eq "п") && "${pf}дм$g" || "дм")})) {
      # (($t eq "п") && "${pf}дм$g" || "дм")
      @wfh = map { my %h = %{$_}; $h{'с'} =~ s/$/и/; \%h } @{$excp};
      goto LABEL1;
    } elsif ($p eq "пм" && defined($excp = $exc->{(($t eq "п") && "${pf}дм$g" || "дм")})) {
      # (($t eq "п") && "${pf}дм$g" || "дм")
  ...
  if (!defined $we) {
    ...
    elsif ($i eq "8" && $wi{'б'} =~ /[${sch}ц]$/) { $we =~ s/^я/а/ }
  }
  ...
  my $ret =
      join(",", map {
        ...
        if (/^[^I0-9()-]+$/) {
          ...
          $_ ? "BAD" : Lingua::RU::Accent::accent($w.$retend,$a,"u").$comment;
        }
      } split(/\/\/|;\s*/, $opp));
