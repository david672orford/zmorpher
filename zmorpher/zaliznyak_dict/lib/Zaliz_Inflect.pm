...
sub decline
  ...
  if(!defined $accent)
    ...
    if($ударное_окончание eq "б")		# stem stressed
      ...
      if($wi{'у1'} =~ /^[ace]/ || $wi{'у1'} eq "b1")
        ...
        print "\$чередование=", defined($чередование) ? $чередование : "", "\n";
  ...
  # FIXME: after this variable contents no longer coorespond to name
  $основа_слова .= $word_ending;
  $основа_слова .= "ся" if(defined $wi{'ся'});
  $wf{'с'} = $основа_слова;

  # Restore word form code (Note though that the saved copy may have itself been modified!)
  $парадигма = $po;
  ...
  # FIXME: why are we switching from hash to list?
  @wfh = (\%wf);

  # If this is a noun in the prepositional singular and it has a second form (в порту),
  if(($парадигма eq 'пе') && ((defined $wi{'п2'}) || (defined $wi{'п2ф'})))
    {
    my $wf1 = inflect($wi, 'де')->[0];			# like the dative
    $wf1->{'с'} =~ s/ё/е/g;
    $wf1->{'у'} = length($wf1->{'с'});			# stress on the last letter
    $wf1->{'п2'} = (defined $wi{'п2'}) && ($wi{'п2'}) || ($wi{'п2ф'});
    push(@wfh, $wf1);
    if(defined $wi{'п2ф'})						# option to use 2nd prepositional fomr
      {
      my %wf2 = %wf;
      $wf2{'п2'} = $wi{'п2ф'};
      push(@wfh, \%wf2);
  ...
  # If this is a noun in the genitive singular, and this noun has a partitive form,
  if(($парадигма eq 'ре') && (defined $wi{'р2'}))
    {
    # Add a partitive form which is the same as the dative.
 ...
 LABEL1:
  ...
  # If we are generating the alternative form of the feminine instrumental case
  # and this is an adjective or a noun in the instrumental singular,
